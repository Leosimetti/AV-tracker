...
def process_mouse_event(event):
    ...
    if event_type is mouse.ButtonEvent:
        insert_data(conn, f"{datetime.fromtimestamp(event.time)}?M?M_CLICK")
        print(f"Mouse clicked at {datetime.fromtimestamp(event.time)}")
    elif event_type is mouse.WheelEvent:
        insert_data(conn, f"{datetime.fromtimestamp(event.time)}?M?M_WHEEL")
        print(f"Mouse wheel moved at {datetime.fromtimestamp(event.time)}")
    else:
        move_count += 1
        if move_count % 100 == 0:
            insert_data(conn, f"{datetime.fromtimestamp(event.time)}?M?M_MOVE")
